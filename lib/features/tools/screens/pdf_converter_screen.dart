...
String _extractDocxTextIsolate(Uint8List bytes) {
  try {
    ...
    for (final p in doc.findAllElements('w:p')) {
      ...
      if (texts.isNotEmpty) buffer.writeln(texts);
    }
...
Future<Uint8List> _generateTextPdf(String text) async {
...
Future<Uint8List> _generateDocxPdf(Map<String, dynamic> args) async {
  final bytes = args['bytes'] as Uint8List;
  final fileName = args['fileName'] as String;
  final text = _extractDocxTextIsolate(bytes);
  final pdf = pw.Document();
  pdf.addPage(pw.MultiPage(
    pageFormat: PdfPageFormat.a4,
    margin: const pw.EdgeInsets.all(40),
    build: (ctx) => [
      pw.Header(level: 2, child: pw.Text(fileName)),
...
Future<Uint8List> _generateImagePdf(Uint8List imageBytes) async {
  final pdf = pw.Document();
...
class _PdfConverterScreenState extends State<PdfConverterScreen> {
  ...
  Future<void> _convert() async {
    setState(() { _loading = true; _done = false; _cancelled = false; });
    try {
      Uint8List result;
      ...
      if (_mode == 0) {
        ...
        result = await compute(_generateTextPdf, text);
      } else if (_mode == 1) {
        if (_fileBytes == null) { setState(() => _loading = false); return; }
        result = await compute(_generateDocxPdf, {
          'bytes': _fileBytes!,
          'fileName': _fileName ?? 'Documento',
        });
      } else {
        if (_fileBytes == null) { setState(() => _loading = false); return; }
        result = await compute(_generateImagePdf, _fileBytes!);
      }

      if (_cancelled) return;
      if (mounted) setState(() { _pdfBytes = result; _done = true; });
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
                  ...
                  children: [
                    ...
                    Row(children: [
                      _ModeChip(label: 'Texto → PDF', icon: Icons.text_fields_rounded, selected: _mode == 0,
                        onTap: _loading ? null : () => setState(() { _mode = 0; _done = false; _fileBytes = null; })),
                      const SizedBox(width: 8),
                      _ModeChip(label: 'Word → PDF', icon: Icons.description_rounded, selected: _mode == 1,
                        onTap: _loading ? null : () => setState(() { _mode = 1; _done = false; _textCtrl.clear(); })),
                      const SizedBox(width: 8),
                      _ModeChip(label: 'Imagem → PDF', icon: Icons.image_rounded, selected: _mode == 2,
                        onTap: _loading ? null : () => setState(() { _mode = 2; _done = false; _textCtrl.clear(); })),
                    ]),
                  ],
                )),
          ...
          if (_loading)
            _ProgressOverlay(color: _color, label: 'Gerando PDF...', onCancel: _cancel),
        ],
      ),
    );
  }
